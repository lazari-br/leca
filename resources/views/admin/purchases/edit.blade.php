    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <form method="POST" action="{{ route('admin.purchases.update', $purchase->id) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="supplier_name" class="block font-medium">Fornecedor</label>
                    <input type="text" name="supplier_name" id="supplier_name" class="w-full border-gray-300 rounded" value="{{ old('supplier_name', $purchase->supplier_name) }}">
                </div>

                <div>
                    <label for="purchase_date" class="block font-medium">Data da Compra</label>
                    <input type="date" name="purchase_date" id="purchase_date" class="w-full border-gray-300 rounded" value="{{ old('purchase_date', $purchase->purchase_date->format('Y-m-d')) }}">
                </div>

                <div>
                    <label for="installments" class="block font-medium">Parcelas</label>
                    <input type="number" name="installments" id="installments" class="w-full border-gray-300 rounded" value="{{ old('installments', $purchase->installments) }}">
                </div>

                <div>
                    <label for="installment_value" class="block font-medium">Valor da Parcela</label>
                    <input type="number" step="0.01" name="installment_value" id="installment_value" class="w-full border-gray-300 rounded" value="{{ old('installment_value', $purchase->installment_value) }}">
                </div>
            </div>
            ...
            <div class="mt-6">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded">Atualizar Compra</button>
            </div>
        </form>
    </div>
