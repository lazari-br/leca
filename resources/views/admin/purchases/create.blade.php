    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <form method="POST" action="{{ route('admin.purchases.store') }}" id="purchase-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="supplier_name" class="block font-medium">Fornecedor <span class="text-red-500">*</span></label>
                    <input type="text" name="supplier_name" id="supplier_name" class="w-full border-gray-300 rounded" placeholder="Nome do fornecedor" value="{{ old('supplier_name') }}" required>
                    <div class="error-message text-red-500 text-sm mt-1 hidden">O campo fornecedor é obrigatório.</div>
                </div>

                <div>
                    <label for="purchase_date" class="block font-medium">Data da Compra <span class="text-red-500">*</span></label>
                    <input type="text" name="purchase_date" id="purchase_date" class="w-full border-gray-300 rounded flatpickr placeholder-gray-400" placeholder="Selecione a data" value="{{ old('purchase_date', date('Y-m-d')) }}" required>
                    <div class="error-message text-red-500 text-sm mt-1 hidden">O campo data da compra é obrigatório.</div>
                </div>

                <div>
                    <label for="installments" class="block font-medium">Parcelas <span class="text-red-500">*</span></label>
                    <input type="number" name="installments" id="installments" class="w-full border-gray-300 rounded" placeholder="Qtd de parcelas" value="{{ old('installments') }}" min="1" required>
            ...
            <div id="items-container">
                <div class="grid grid-cols-1 md:grid-cols-6 gap-4 item-row mb-4">
                    <div>
                        ...
                        <select name="items[0][product_id]" class="product-select w-full border rounded" data-index="0" required>
                            <option value="">Selecione código</option>
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" data-variations='@json($product->variations)' data-price="{{ $product->price }}" data-name="{{ $product->name }}">
                    ...
                    <div>
                        ...
                        <input type="text" class="product-name w-full border-gray-300 rounded bg-gray-100" placeholder="Nome do produto" readonly>
                    </div>

                    <div>
                        <label class="block font-medium">Variação <span class="text-red-500">*</span></label>
                        <select name="items[0][product_variation_id]" id="variation-0" class="variation-select w-full border rounded" data-index="0" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="items[0][quantity]" class="w-full border-gray-300 rounded quantity" value="1" min="1" required>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="items[0][unit_price]" class="w-full border-gray-300 rounded unit-price bg-gray-100" readonly>
                    </div>

                    <div class="flex items-end">
                        <button type="button" class="bg-red-500 text-white px-3 py-2 rounded remove-item hidden">Remover</button>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6">
                <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded">Salvar Compra</button>
            </div>
        </form>
    </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function validateForm() {
                ...
                const requiredFields = ['supplier_name', 'purchase_date', 'installments', 'payment_method'];
